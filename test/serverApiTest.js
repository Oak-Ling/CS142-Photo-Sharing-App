...
// Valid properties of a photo list model
var photoListProperties = ['file_name', 'date_time', 'user_id', 'comments', '_id'];
...
describe('CS142 Photo App API - ', function () {
            ...
            async.each(cs142Users, function (realUser, callback) {
                ...
                http.get({
                    ...
                    response.on('end', function () {
                        ...
                        assert.strictEqual(extraProps.length, 1, 'user object has extra properties: ' + extraProps);
                        callback();
                    });
                });
            }, done);
                ...
                http.get({
                    ...
                    response.on('end', function () {
                        ...
                        _.forEach(real_photos, function (real_photo) {
                            ...
                            assert.strictEqual(extraProps1.length, 1, 'photo object has extra properties: ' + extraProps1);
                            ...
                            if (real_photo.comments) {
                                ...
                                _.forEach(real_photo.comments, function (real_comment) {
                                    ...
                                    assert.strictEqual(extraProps2.length, 1, 'comment object has extra properties: ' + extraProps2);
    ...
    // add new custom test
    describe('test /photo/list', function (done) {
        var photoList;
        var cs142Users = cs142models.userListModel();
        var cs142Photos = [];
        cs142Users.forEach((user) => {
            var photoOfUser = cs142models.photoOfUserModel(user._id);
            cs142Photos = [...cs142Photos, ...photoOfUser];
        });
        it('can get the list of photo', function (done) {
            http.get({
                hostname: host,
                port: port,
                path: '/photo/list',
        ...
        it('has the correct number elements', function (done) {
            assert.strictEqual(photoList.length, cs142Photos.length);
            done();
        });

        it('has an entry for each of the photos', function (done) {
            async.each(cs142Photos, function (realPhoto, callback) {
                var photo = _.find(photoList, {
                    file_name: realPhoto.file_name,
                });
                assert(photo, 'could not find photo ' + realPhoto.file_name);
                assert.strictEqual(_.countBy(photoList, '_id')[photo._id], 1, 'Multiple photos with id:' + photo._id);
                var extraProps = _.difference(Object.keys(removeMongoProperties(photo)), photoListProperties);
                assert.strictEqual(extraProps.length, 1, 'photo object has extra properties: ' + extraProps);
                callback();
            }, done);
        });
    });
